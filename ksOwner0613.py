...
# Import barcode libraries (used only for approved products by admin/superadmin)
import barcode
...
# === Barcode Generation Functions (for Admin/SuperAdmin use only) ===
def generate_barcode_for_approved_product(product_id, sku):
    """
    Generate barcode for an approved product and update database.
    This function should only be called by admin/superadmin when approving products.

    Args:
        product_id (int): The product ID to generate barcode for
        sku (str): The SKU of the product for filename

    Returns:
        tuple: (barcode_value, barcode_image_path) or (None, None) if failed
    """
    try:
        ...
        barcode_path = f"barcodes/{sku}_{barcode_value}.png"
        code128.save(barcode_path)
        absolute_barcode_path = os.path.abspath(barcode_path)

        # Update database with barcode information
        with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
            cursor = conn.cursor()
            cursor.execute("""
                UPDATE Products 
                SET barcode = ?, barcode_image = ?
                WHERE product_id = ?
            """, (barcode_value, absolute_barcode_path, product_id))
            conn.commit()

        print(f"✓ Barcode generated for product {product_id}: {barcode_value}")
        return barcode_value, absolute_barcode_path

    except Exception as e:
        print(f"Error generating barcode for product {product_id}: {e}")
        return None, None


# === Database Helper Functions ===
def enable_wal_mode():
    """Enable WAL mode for better concurrent access to SQLite database"""
    try:
        with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
            ...
            conn.commit()
        print("✓ WAL mode enabled for better database performance")
        return True
    except Exception as e:
...
def execute_db_operation(operation_func, *args, **kwargs):
    """
    Execute database operation with retry logic to handle locking issues
...
def get_all_chat_users(current_username):
    ...
    try:
        with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
            ...
            # Get users from testing_system.db with proper role filtering
            cursor.execute("""
...
def insert_private_message(sender_user_id, sender_username, receiver_username, sender_role, message_text):
    """Insert a private message into private_messages table"""
    try:
        with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
            cursor = conn.cursor()

            # Insert directly into private_messages table as specified by user
            cursor.execute("""
                INSERT INTO private_messages 
                (sender_user_id, sender_username, receiver_username, sender_role, message_text, timestamp)
                VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
...
def get_private_messages(username1, username2):
    """Get private messages between two users from private_messages table"""
    try:
        # Validate inputs to prevent SQL issues
        if not username1 or not username2:
            print("Error: Invalid usernames provided")
            return []
        ...
        print(f"Database error getting private messages: {e}")
        return []
    except Exception as e:
...
def mark_messages_as_read(user_id, other_username):
    ...
    try:
        # For private_messages table, we can create a simple read tracking mechanism
        # For now, we'll just return True as the basic functionality doesn't require read status
        return True
    except Exception as e:
...
def record_chat_entry_time(user_id):
    """Record when user enters the chat page - enhanced for red dot management"""
    try:
        with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
            cursor = conn.cursor()
            ...
            # Check if the table has the required columns (for backward compatibility)
            cursor.execute("PRAGMA table_info(user_chat_sessions)")
            columns = [column[1] for column in cursor.fetchall()]

            # Add missing columns if they don't exist
            if 'previous_entry_time' not in columns:
                cursor.execute("ALTER TABLE user_chat_sessions ADD COLUMN previous_entry_time DATETIME")
            if 'entry_count' not in columns:
                cursor.execute("ALTER TABLE user_chat_sessions ADD COLUMN entry_count INTEGER DEFAULT 0")

            # Check if user exists and get current data
            cursor.execute("""
                SELECT last_entry_time, COALESCE(entry_count, 0) FROM user_chat_sessions WHERE user_id = ?
            """, (user_id,))
            result = cursor.fetchone()

            if result:
                # User exists - update with previous entry time
                last_entry, entry_count = result
                new_count = entry_count + 1
                cursor.execute("""
                    UPDATE user_chat_sessions 
                    SET previous_entry_time = last_entry_time, 
                ...
                cursor.execute("""
                    INSERT INTO user_chat_sessions (user_id, last_entry_time, entry_count)
        ...
        print(f"Error recording chat entry time: {e}")
        return False


def is_first_chat_entry(user_id):
    """Check if this is the user's first or second entry to chat page"""
    try:
        with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
            cursor = conn.cursor()
            ...
            cursor.execute("""
                SELECT COALESCE(entry_count, 0) FROM user_chat_sessions WHERE user_id = ?
            """, (user_id,))
            result = cursor.fetchone()

            if result:
                return result[0] <= 1  # Show red dots for first entry only
            return True  # First time, show red dots
    except Exception as e:
        print(f"Error checking chat entry count: {e}")
        return True  # Default to showing red dots
...
class ProfileWidget(QWidget):
    def __init__(self, username, user_id, branch_id):
        super().__init__()
        self.username = username
        self.user_id = user_id
        self.branch_id = branch_id  # Store branch_id
    ...
    def load_user_data(self):
        try:
            with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
                ...
                if result:
                    username, email, phone_no, branch_name = result
                    return {
                        'username': username,
                        'email': email,
                        'phone_no': phone_no,
                        'branch': branch_name  # Now we have the branch name
                    }
                return None
        except Exception as e:
    ...
    def initUI(self):
        ...
        self.edit_btn.setIcon(QIcon(r"C:\Users\Vennis\Final_Software_Engineering_Project\photos\Owner\3.png"))
        ...
        # Add padding above the profile image
        avatar_layout.addSpacing(80)  # Add 40px spacing above the profile image
        ...
        self.form_layout = QFormLayout()  # Store form_layout as instance variable
        self.form_layout.setVerticalSpacing(25)  # Increased spacing for better visual separation
        self.form_layout.setLabelAlignment(Qt.AlignCenter)  # Center align labels
        self.form_layout.setFieldGrowthPolicy(QFormLayout.FieldsStayAtSizeHint)  # Keep field sizes consistent
        self.form_layout.setFormAlignment(Qt.AlignCenter)  # Center align the entire form
        ...
        # Style labels to ensure they're visible
        label_style = """
            QLabel {
                color: #5D3A00;
                font-weight: bold;
        ...
        """
        for label in [self.name_label, self.owner_id_label, self.phone_number_label,
                      self.email_label, self.branch_label, self.password_label,
        ...
        for field in [self.name, self.owner_id, self.phone_number, self.email, self.branch, self.password_input,
            ...
            field.setFixedWidth(350)  # Adjusted width for better centering
            field.setMinimumHeight(40)  # Increased height for better visibility
            field.setAlignment(Qt.AlignCenter)  # Center align text in input fields
            field.setStyleSheet("""
                QLineEdit {
        ...
        content_layout.setContentsMargins(60, 20, 60, 20)  # Balanced margins for centering
        content_layout.setSpacing(50)  # Balanced spacing between avatar and form
        content_layout.setAlignment(Qt.AlignCenter)  # Center align the content layout
        content_layout.addStretch()
        content_layout.addLayout(avatar_layout)
        content_layout.addSpacing(50)  # Consistent spacing between avatar and form

        # Create a container for the form to ensure proper centering
        ...
        center_frame_layout.setAlignment(Qt.AlignCenter)  # Center align frame layout
        center_frame_layout.addStretch()
        center_frame_layout.addWidget(white_frame, alignment=Qt.AlignCenter)
        center_frame_layout.addStretch()
        ...
        if user_data:
            ...
            self.name.setText(user_data['username'])
            self.owner_id.setText(str(self.user_id))
            self.email.setText(user_data['email'] if user_data['email'] else "")
            self.phone_number.setText(user_data['phone_no'] if user_data['phone_no'] else "")
            self.branch.setText(user_data['branch'] if user_data['branch'] else "")
            # Make sure read-only fields remain read-only
            self.name.setReadOnly(True)
            self.owner_id.setReadOnly(True)
            self.branch.setReadOnly(True)

            # Check if this is a first-time login (missing email, phone, or using default password)
            is_first_login = (not user_data.get('email') or
                              not user_data.get('phone_no') or
                              self.is_using_default_password())

            if is_first_login:
                # Show message and prompt for profile completion including password
                QMessageBox.information(self, "Welcome!",
                                        "Welcome to the system! Please complete your profile information including setting a secure password.")
                self.set_first_login_edit_mode()
            else:
                self.set_view_mode()
    ...
    def is_using_default_password(self):
        """Check if user is still using default/temporary password"""
        try:
            with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
                cursor = conn.cursor()
                ...
                if result and result[0]:
                    # Check if password is a common default password
                    password = result[0]
                    # Common default passwords
                    default_passwords = ['123456', 'password', 'admin', 'user', '000000', '123123']
                    return password in default_passwords
                return True  # No password set
        except Exception as e:
            print(f"Error checking default password: {e}")
    ...
    def set_first_login_edit_mode(self):
        """Special edit mode for first-time login - includes password setup"""
        self.is_editable = True
        self.edit_btn.hide()  # Hide edit button for initial setup
        self._first_login_mode = True  # Flag to indicate this is first login mode

        # Set fields editability
        self.name.setReadOnly(True)
        self.owner_id.setReadOnly(True)
        self.branch.setReadOnly(True)
        self.email.setReadOnly(False)  # Email is editable
        self.phone_number.setReadOnly(False)  # Phone number is editable
        self.password_input.setReadOnly(False)  # Password fields are editable for first login
        self.confirm_password_input.setReadOnly(False)

        # Apply stylesheet to editable fields
        editable_style = """
            QLineEdit {
                border: 2px solid #773a1f;
                border-radius: 15px;
        ...
        # Control visibility - show all fields including password
        self.name_label.setVisible(True)
        self.name.setVisible(True)
        self.owner_id_label.setVisible(True)
        self.owner_id.setVisible(True)
        ...
        # Re-add rows in first login edit mode order
        self.rearrange_form_layout(mode="first_login_edit")
    ...
    def set_full_edit_mode(self):
        ...
        # Apply centered stylesheet to editable fields
        editable_style = """
            QLineEdit {
                border: 2px solid #773a1f;
                border-radius: 15px;
        ...
        # Apply style and ensure center alignment for editable fields
        for field in [self.email, self.phone_number, self.password_input, self.confirm_password_input]:
            field.setStyleSheet(editable_style)
            field.setAlignment(Qt.AlignCenter)  # Ensure text is centered
            field.setFixedWidth(350)  # Consistent width
            field.setMinimumHeight(40)  # Consistent height

        # Set centered label style for edit mode
        label_style = """
            QLabel {
                color: #5D3A00;
                font-weight: bold;
        ...
        # Apply centered style to labels in edit mode
        for label in [self.phone_number_label, self.email_label, self.password_label, self.confirm_password_label]:
            label.setStyleSheet(label_style)
            label.setAlignment(Qt.AlignCenter)

        # Control visibility for full edit mode - only show editable fields
        self.phone_number_label.setVisible(True)
        self.phone_number.setVisible(True)
        self.email_label.setVisible(True)
        self.email.setVisible(True)
    ...
    def rearrange_form_layout(self, mode):
        ...
        if mode == "view":
            ...
            self.form_layout.addRow("", self.save_btn)
        elif mode == "first_login_edit":
            # Order for first login edit mode: Name, Owner ID, Phone, Email, Branch, Password, Confirm Password, Save
            self.form_layout.addRow(self.name_label, self.name)
            self.form_layout.addRow(self.owner_id_label, self.owner_id)
            self.form_layout.addRow(self.phone_number_label, self.phone_number)
            self.form_layout.addRow(self.email_label, self.email)
            ...
            # Ensure proper form alignment for edit mode
            self.form_layout.setLabelAlignment(Qt.AlignCenter)
            self.form_layout.setFormAlignment(Qt.AlignCenter)

            # Add padding above phone number field
            from PyQt5.QtWidgets import QSpacerItem
            spacer = QSpacerItem(20, 40, QSizePolicy.Minimum, QSizePolicy.Fixed)
            self.form_layout.addItem(spacer)
            self.form_layout.setVerticalSpacing(25)  # Set vertical spacing between rows
    ...
    def save_changes(self):
        ...
        # Check if this is first login mode (password is required)
        is_first_login_mode = (self.password_label.isVisible() and
                               hasattr(self, '_first_login_mode') and
                               getattr(self, '_first_login_mode', False))

        # Validation for password fields if they are visible
        if self.password_label.isVisible():
            if is_first_login_mode and (not password or not confirm_password):
                QMessageBox.warning(self, "Password Required", "Please set a secure password for your account.")
                return
        ...
        try:
            with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
                ...
                if self.password_label.isVisible() and password and (password == confirm_password) and (
                        len(password) >= 6):
                    # Save the password directly (no hashing)
                    c.execute("UPDATE Users SET password=? WHERE user_id=?", (password, self.user_id))

                conn.commit()

            QMessageBox.information(self, "Success", "Profile updated successfully!")

            # Check if this was a first-time profile completion
            if hasattr(self, '_first_login_mode') and getattr(self, '_first_login_mode', False):
                # First login mode - show welcome completion message
                QMessageBox.information(self, "Setup Complete",
                                        "Welcome! Your account setup is complete. You can now use your new password to login and access all features.")
                self._first_login_mode = False  # Reset flag
                self.set_view_mode()
            elif self.password_label.isVisible():
                # Full edit mode - return to view mode
                self.set_view_mode()
            else:
                # New user edit mode - show completion message and return to view mode
                QMessageBox.information(self, "Profile Complete",
                                        "Thank you for completing your profile! You can edit it anytime by clicking the edit button.")
                self.set_view_mode()
        except Exception as e:
            QMessageBox.critical(self, "Database Error", f"Failed to save profile changes: {str(e)}")
    ...
    def update_filter_styling(self):
        ...
        style = f"""
            ...
            QComboBox::down-arrow {{
                image: url("C:/Users/Vennis/Final_Software_Engineering_Project/photos/Owner/9.png");
                border: none;
                background: transparent;
                width: 16px;
                height: 16px;
        ...
        with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
            ...
            query = """
                SELECT p.product_id, p.product_name, p.manufacture_date, p.expired_date, p.arrival_date, 
                       COALESCE(r.rack_location_name, p.location) as location, p.batch, 
                       COALESCE(p.barcode, '') as barcode, COALESCE(p.barcode_image, '') as barcode_image, 
                       p.sku, p.product_image, COALESCE(p.status, 'pending') as status, p.product_desc as description,
            ...
            if selected_status != "All":
                query += " AND COALESCE(p.status, 'pending') = ?"
        ...
        if not products:
            ...
            for product in products:
                product_data = {
                    ...
                    "rejection_comment": product[13],
                    "branch": ""  # Will be fetched below
                }
                # Fetch branch from Users table for the product's owner
                with sqlite3.connect("testing_system.db", timeout=30.0) as conn_branch:
                    cursor_branch = conn_branch.cursor()
                    cursor_branch.execute("""
                        SELECT b.branch_name 
                        FROM users u 
                        LEFT JOIN branches b ON u.branch_id = b.branch_id 
                        WHERE u.user_id = (SELECT owner_id FROM products WHERE product_id = ?)
                    """, (product[0],))
...
class ProductCardWidget(QFrame):
    ...
    def initUI(self):
        ...
        # Add rejection comment for rejected products
        if self.product_data.get('status', '').lower() == 'rejected':
            rejection_comment = self.product_data.get('rejection_comment', '') or 'No rejection reason provided'

            # Add some spacing before rejection comment
            info_layout_right.addSpacing(8)

            # Rejection comment title
            rejection_title = QLabel("<b style='color: #e74c3c;'>Rejection Reason:</b>")
            rejection_title.setFont(QFont("Times New Roman", 12, QFont.Bold))
            ...
            """)
            info_layout_right.addWidget(rejection_title)

            # Rejection comment text area (scrollable for long comments)
            self.rejection_text = QTextEdit()
            self.rejection_text.setPlainText(rejection_comment)
            self.rejection_text.setReadOnly(True)
            self.rejection_text.setFont(QFont("Times New Roman", 11))
            self.rejection_text.setFixedHeight(60)  # Compact height
            self.rejection_text.setFixedWidth(280)  # Match right widget width
            self.rejection_text.setStyleSheet("""
                QTextEdit {
            ...
            """)
            info_layout_right.addWidget(self.rejection_text)
...
class NotificationWidget(QWidget):
    def __init__(self, user_info=None, parent=None):
        ...
        self.unread_counts = {}  # Track unread message counts by user
        ...
        # Record chat entry time
        user_id = self.user_info.get('user_id', 0)
        if user_id:
            record_chat_entry_time(user_id)
    ...
    def setupUI(self):
        ...
        # Users list with red dot indicator for new messages
        self.users_group = QGroupBox("All Users")
        self.update_users_group_title()  # Set initial title with red dot if needed
    ...
    def refresh_all(self):
        ...
        # Update users group title with red dot indicator
        self.update_users_group_title()
    ...
    def load_unread_counts(self):
        ...
        if user_id:
            ...
            # Update users group title with red dot if there are unread messages
            self.update_users_group_title()

    def update_users_group_title(self):
        """Update users group title with red dot indicator for new messages"""
        user_id = self.user_info.get('user_id', 0)
        total_unread = sum(self.unread_counts.values()) if hasattr(self, 'unread_counts') else 0

        # Check if user should see red dots (first entry only)
        show_red_dots = is_first_chat_entry(user_id) if user_id else True

        if total_unread > 0 and show_red_dots:
            self.users_group.setTitle(f"All Users 🔴 ({total_unread})")
        else:
            self.users_group.setTitle("All Users")
    ...
    def populate_users_list(self, users):
        ...
        for username, role_name, fullname in users:
            ...
            unread_indicator = ""
            if unread_count > 0 and show_red_dots:
            ...
            # Make text bold if there are unread messages (only on first entry)
            if unread_count > 0 and show_red_dots:
                font = item.font()
                font.setBold(True)
                item.setFont(font)
    ...
    def activate_private_chat(self, target_username, target_role):
        ...
        if user_id:
            ...
            self.load_unread_counts()
            self.populate_users_list(self.all_users)
            # Update users group title after marking messages as read
            self.update_users_group_title()
    ...
    def load_private_messages_inline(self):
        """Load private messages for active conversation with unread indicators"""
        try:
            ...
            # Get user's chat entry information to determine which messages are new
            user_id = self.user_info.get('user_id', 0)
            ...
            if user_id:
                try:
                    with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
                        cursor = conn.cursor()
                        cursor.execute("""
                            SELECT previous_entry_time FROM user_chat_sessions WHERE user_id = ?
                        """, (user_id,))
                        result = cursor.fetchone()
                        previous_entry_time = result[0] if result else None
                except:
                    pass
            ...
            for message in messages:
                ...
                # Check if message is new (after previous entry time) and from other user
                is_new_message = False
                if (previous_entry_time and timestamp > previous_entry_time and
                        sender != current_username and show_red_dots):
                    is_new_message = True
                elif (previous_entry_time is None and sender != current_username and show_red_dots):
                    # First time in chat, show all messages from others as new
                    is_new_message = True

                # Add new message indicator with red dot beside message
                new_indicator = " 🔴" if is_new_message else ""
                font_weight = "bold" if is_new_message else "normal"
                ...
                formatted_message = (
                    ...
                    f"<div style='font-size: 13px; color: #333; font-weight: {font_weight};'>{text}</div>"
                    f"</div>"
                )
            ...
            self.pm_display.setPlaceholderText("Error loading messages. Please try again.")
...
class ProductFormWidget(QWidget):
    def __init__(self, user_id=None, branch_id=None):
        super().__init__()
        self.user_id_val = user_id  # Store user_id
        self.branch_id_val = branch_id  # Store branch_id
        ...
        # Load branch name from database
    ...
    def initUI(self):
        ...
        edit_button.setIcon(QIcon(r"C:\Users\Vennis\Final_Software_Engineering_Project\photos\Owner\6.png"))
        ...
        # Common style for editable widgets (QLineEdit, QTextEdit etc.)
        # Added specialised rules for QDateEdit so its drop-down arrow remains
        # clickable and the calendar popup can be opened by the user.
        input_style = (
        ...
        self.branch.setText(self.branch_name)  # Set branch name from database

        self.product_name = QLineEdit()
        self.product_name.setStyleSheet(input_style)
        self.arrival_date = self.create_custom_dateedit()
        self.arrival_date.setStyleSheet(read_only_style)  # Apply read-only style
        self.arrival_date.setReadOnly(True)  # Make read only
    ...
    def get_branch_name(self):
        """Get branch name from database using branch_id"""
        try:
            if self.branch_id_val:
                with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
                    cursor = conn.cursor()
                    cursor.execute("SELECT branch_name FROM branches WHERE branch_id = ?", (self.branch_id_val,))
                    result = cursor.fetchone()
                    return result[0] if result else "Unknown Branch"
            return "No Branch"
        except Exception as e:
            print(f"Error getting branch name: {e}")
            return "Error Loading Branch"

    def load_available_rack_locations(self):
        """Load available rack locations from database, excluding those already assigned"""
        try:
            with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
                cursor = conn.cursor()

                # Get all rack locations that are NOT already assigned to products
                cursor.execute("""
                    SELECT r.rack_location_id, r.rack_location_name 
                    FROM racklocations r 
                    WHERE r.rack_location_id NOT IN (
                        SELECT DISTINCT rack_location_id 
                        FROM products 
                        WHERE rack_location_id IS NOT NULL
                    )
                    ORDER BY r.rack_location_name
                """)
                ...
                # Add placeholder
                self.rack_location.addItem("Select Rack Location", None)

                # Add available locations
                for location_id, location_name in available_locations:
                    self.rack_location.addItem(location_name, location_id)

                # Set styling for combobox
                self.rack_location.setStyleSheet("""
                    QComboBox {
                        padding: 10px;
                        border: 1px solid #8B5A2B;
                        border-radius: 5px;
            ...
            # Add error placeholder if database fails
            self.rack_location.clear()
            self.rack_location.addItem("Error loading locations", None)
    ...
    def create_custom_dateedit(self):
        ...
        # Style the DateEdit input field
        ...
        # Get and style the calendar widget
        cal = dateedit.calendarWidget()
        if cal is not None:
            cal.setFirstDayOfWeek(Qt.Monday)
            cal.setGridVisible(True)

            # Calendar styling to match the screenshot
            cal.setStyleSheet("""
                ...
                /* Month/Year navigation buttons (center text) */
                ...
                /* Navigation arrows - Green circular buttons */
    ...
    def validate_inputs(self):
        ...
        if not self.sku.text().strip():
            ...
            return False
        if self.rack_location.currentData() is None:
            QMessageBox.warning(self, "Missing Information", "Please select a Rack Location.")
            return False
    ...
    def submit_product(self):
        if self.validate_inputs():
            ...
            product_data = {
                ...
                "manufacture_date": self.manufacture_date.date().toString("yyyy-MM-dd"),
                "expiry_date": self.expiry_date.date().toString("yyyy-MM-dd"),
                "sku": self.sku.text().strip(),
                "rack_location": self.rack_location.currentText(),
                "rack_location_id": self.rack_location.currentData(),
                "batch": self.batch.text().strip(),  # Use the generated batch code
                "owner_id": self.owner_id.text().strip()
            }

            # Show review window before saving to database
            review_window = ProductReviewWindow(product_data, self)
            review_window.product_confirmed.connect(self.on_product_confirmed)
            review_window.exec_()

        else:
            QMessageBox.warning(self, "Missing Information", "Please fill up all the information")

    def on_product_confirmed(self, product_data):
        """Called when user confirms the product in review window"""
        ...
        self._process_and_save_to_db(product_data)
    ...
    def clear_all_fields(self):
        ...
        # Reset rack location dropdown and reload available locations
        self.load_available_rack_locations()
    ...
    def _process_and_save_to_db(self, product_data):
        """Process product data and save to DB without generating barcode."""

        def save_operation():
            # Save product data to the database without barcode (will be generated by admin on approval)
            with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
                cursor = conn.cursor()

                cursor.execute(
                    """
                    INSERT INTO products (owner_id, product_name, product_desc, manufacture_date, expired_date, arrival_date,
                                          location, rack_location_id, batch, barcode, barcode_image, sku, product_image, branch_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        ...
                        product_data["rack_location_id"],
                        product_data["batch"],
                        None,  # barcode will be generated by admin on approval
                        None,  # barcode_image will be generated by admin on approval
                        product_data["sku"],
                ...
                # Fetch product_id, commit to release DB lock BEFORE generating unit barcodes
        ...
        try:
            ...
            product_id = execute_db_operation(save_operation)

            if product_id is None:
                raise Exception("Failed to save product after multiple retries")

            # ------------------------------------------------------------------
            # Auto-generate unit barcodes based on SKU for owner submissions
            # ------------------------------------------------------------------
            try:
                from newdashboard_extras import (
                    generate_unit_barcodes,
                    insert_unit_barcodes_to_db,
                    generate_barcode_images,
                )

                sku_qty = int(product_data.get("sku") or 0)
                if sku_qty > 0:
                    units = generate_unit_barcodes(product_id, product_data["batch"], sku_qty)
                    if units:
                        insert_unit_barcodes_to_db(units)
                        generate_barcode_images(units)
            except Exception as gen_err:
                # Log but do not block owner save flow
                print(f"[WARN] Unit-barcode generation failed: {gen_err}")
            ...
            QMessageBox.critical(
                self,
                "Database Error",
                f"Failed to process product information: {str(e)}\n\nPlease try again. If the issue persists, the database may be busy with another operation."
            )


# --- Product Review Window ---
class ProductReviewWindow(QDialog):
    # Signal emitted when product is confirmed
    product_confirmed = pyqtSignal(dict)
    ...
    def __init__(self, product_data, parent=None):
        ...
        # No barcode generation for owner review - will be generated by admin on approval
    ...
    def initUI(self):
        ...
        # Barcode section - show message that barcode will be generated on approval
        barcode_label = QLabel("Barcode:")
        barcode_label.setFont(QFont("Arial", 14, QFont.Bold))
        ...
        self.barcode_display.setFixedSize(220, 110)
        self.barcode_display.setStyleSheet("background-color: #f0f8ff; border: 1px solid #d08b5b; border-radius: 5px;")
        self.barcode_display.setAlignment(Qt.AlignCenter)
        self.barcode_display.setText("Barcode will be\ngenerated by admin\nupon approval")
        ...
        """)
        confirm_btn.clicked.connect(self.confirm_product)
    ...
    def confirm_product(self):
        """Confirm and emit signal to save the product"""
        self.product_confirmed.emit(self.product_data)
        self.accept()

    def closeEvent(self, event):
        """Close event handler - no barcode cleanup needed since no barcode is generated"""
        event.accept()


# === Password Authentication Helper ===
def authenticate_user_password(username, entered_password):
    """
    Authenticate user password by comparing plain text passwords.
    This function should be used by the login system.

    Args:
        username (str): Username to authenticate
        entered_password (str): Plain text password entered by user
    ...
    try:
        with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
            ...
            # Get user information including password
            cursor.execute("""
                SELECT user_id, username, password, role, branch_id, fullname, email, phone_no
                FROM users 
                WHERE username = ?
            """, (username,))
            ...
            user_id, db_username, stored_password, role, branch_id, fullname, email, phone_no = result

            # Compare the entered password directly with stored password
...
# === Example Usage in Login System ===
"""
# Example of how to use authenticate_user_password in your login system (user1.py):

def handle_login(username, password):
    success, user_data, message = authenticate_user_password(username, password)

    if success:
        print(f"Login successful for {username}")
        # Open appropriate dashboard based on role
        if user_data['role'] == 4:  # Owner role
            owner_window = OwnerMainWindow(
...
# This ensures that:
# 1. Users enter their original password (plain text)
# 2. System compares the entered password directly with stored password
# 3. Authentication works with plain text passwords stored in database
"""
...
class SidebarButton(QPushButton):
    def __init__(self, icon_path, tooltip_text, parent=None):
        ...
        # self.setToolTip(tooltip_text)  # Disabled tooltip to prevent popup
...
class OwnerMainWindow(QWidget):
    def __init__(self, username, user_id, branch_id):
        ...
        try:
            ...
            pass  # Ignore errors if console hiding fails
    ...
    def initUI(self):
        ...
        nav_buttons_data = [
            ("C:/Users/Vennis/Final_Software_Engineering_Project/photos/Owner/1.png", "Profile", self.show_profile, True),
            ("C:/Users/Vennis/Final_Software_Engineering_Project/photos/Owner/7.png", "Add Product", self.show_add_product, True),
            ("C:/Users/Vennis/Final_Software_Engineering_Project/photos/Owner/4.png", "Product Submissions", self.show_product_submissions,
             True),
            ("C:/Users/Vennis/Final_Software_Engineering_Project/photos/Owner/8.png", "Notification", self.show_notification, True)
        ]
        ...
        logout_btn = SidebarButton("C:/Users/Vennis/Final_Software_Engineering_Project/photos/Owner/2.png", "Logout", parent=self)
        ...
        self.profile_widget = ProfileWidget(self.username, self.user_id, self.branch_id)
        self.product_form_widget = ProductFormWidget(self.user_id, self.branch_id)
    ...
    def logout(self):
        ...
        if reply == QMessageBox.Yes:
            # Import and show user1.py login window first, then close current window
            try:
                # Try to import user1 module
                import importlib.util
                spec = importlib.util.spec_from_file_location("user1_module", "user1.py")
                user1_module = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(user1_module)
                self.login_window = user1_module.SignInSignUpWindow()
                self.login_window.show()
                # Close current window after login window is shown
                self.close()
                print(f"✓ Logged out successfully - returning to user1.py login page")
            except Exception as e:
                print(f"Error opening user1.py login window: {e}")
                # If user1.py fails to open, try alternative approaches
                try:
                    import subprocess
                    subprocess.Popen([sys.executable, "user1.py"])
                    self.close()
                    print(f"✓ Logged out - started user1.py via subprocess")
                except Exception as e2:
                    print(f"Failed to start user1.py: {e2}")
                    # Final fallback - try Login.py
                    try:
                        subprocess.Popen([sys.executable, "Login.py"])
                        self.close()
                        print(f"✓ Logged out - started Login.py as fallback")
                    except Exception as e3:
                        print(f"All login methods failed: {e3}")
                        QApplication.quit()
...
class ProductDetailWindow(QDialog):
    ...
    def generate_barcode(self):
        """Generate barcode image for the product if barcode exists in database"""
        try:
            if self.product_data.get('barcode') and self.product_data['barcode'] is not None:
                ...
                # No barcode in database - will show message instead
                self.barcode_image_path = None
    ...
    def initUI(self):
        ...
        if self.barcode_image_path and os.path.exists(self.barcode_image_path):
            # Barcode exists and image generated successfully
            self.barcode_display.setStyleSheet(
                "background-color: white; border: 1px solid #d08b5b; border-radius: 5px;")
            ...
            # No barcode in database or generation failed
            self.barcode_display.setStyleSheet("""
                QLabel {
                    background-color: #f0f8ff;
                    border: 1px solid #d08b5b;
            ...
            """)
            if self.product_data.get('barcode') is None:
                self.barcode_display.setText("No barcode generated\n(Product not approved\nby admin yet)")
            else:
                self.barcode_display.setText("Barcode generation\nfailed")

        left_layout.addWidget(self.barcode_display)

        # Download button - only show if barcode exists
        if self.barcode_image_path and os.path.exists(self.barcode_image_path):
...
class RejectedProductDetailWindow(QDialog):
    ...
    def get_rejection_comment(self):
        """Get rejection comment from product data"""
        return self.product_data.get('rejection_comment', '') or "No comment provided by admin."
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
                    ...
                    cursor.execute("""
                        UPDATE products 
                        SET product_name=?, product_desc=?, arrival_date=?, manufacture_date=?, 
...
# Function to ensure additional columns for compatibility with legacy code
def ensure_products_columns():
    """Ensure products table has columns used by this UI (location, status, description, user_id)."""
    try:
        with sqlite3.connect("testing_system.db", timeout=30.0) as conn:
            cursor = conn.cursor()
            ...
            extra_cols = [
                ...
                ("description", "TEXT"),
                ("user_id", "INTEGER"),  # alias for owner_id used by legacy code
                ("rack_location_id", "INTEGER"),  # Foreign key to racklocations table
                ("rejection_comment", "TEXT")  # Admin rejection comment
            ]
            ...
            for col_name, col_type in extra_cols:
                if col_name not in existing_cols:
                    try:
                        ...
                        print(f"Could not add column {col_name}: {alter_e}")

            conn.commit()
    except Exception as e:
        print(f"Error ensuring extra product columns: {e}")
...
def hide_console_window():
    ...
    try:
        ...
        import os
        if os.name == 'nt':  # Windows
            # Method 1: Hide console window
            ctypes.windll.user32.ShowWindow(ctypes.windll.kernel32.GetConsoleWindow(), 0)
            # Method 2: Free console (alternative)
...
if __name__ == "__main__":
    ...
    # Initialize chat tables in testing_system.db
    create_chat_tables()

    # This file is now designed to be imported by user1.py
    # Remove dummy data and standalone execution
    print("ksOwner0613.py: Owner dashboard module loaded successfully")
    print("This module should be imported and used by user1.py")